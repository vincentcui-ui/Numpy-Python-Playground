...
class NumpyBasics:
    def split_list(self, x: List) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # >>> YOUR CODE HERE >>>
        x_array = np.array(x)
        split = len(x_array) // 2 + len(x_array) % 2
        first_half = x_array[:split]
        second_half = x_array[split:]
    ...
    def custom_normalization(self, x: np.ndarray) -> np.ndarray:
        ...
        # >>> YOUR CODE HERE >>>
        min_val = np.min(x)
        x_mod = np.where(x < 0, min_val, x)
        norm_array = x_mod * x
        array = np.sqrt(norm_array)
        # <<< END OF YOUR CODE <<<
